      ...
      if (feature.getGeometry().getType() == "LineString") {
        return new ol.style.Style({
          text: new ol.style.Text({
            text: feature.getGeometry().getLength()
              ? app.measure.formatLength(feature.getGeometry().getLength())
              : "",
  ...
  basemaps: {
    osm: new ol.layer.Tile({
      source: new ol.source.XYZ({
        ...
        attributions:
          "© <a href='http://openstreetmap.org' class='external'>OpenStreetMap</a> contributors, © <a href='https://carto.com/attribution' class='external'>CARTO</a>",
      }),
    }),
    nysdop: new ol.layer.Tile({
      source: new ol.source.XYZ({
        ...
        attributions:
          "<a href='http://orthos.dhses.ny.gov/arcgis/rest/' class='external'>NYS</a>",
        params: {
          LAYERS: "0,1,2,3,4",
          TILED: true,
        },
    ...
    topo: new ol.layer.Tile({
      source: new ol.source.XYZ({
        ...
        attributions:
          "<a href='https://www.doi.gov' class='external'>USDOI</a> | <a href='https://www.usgs.gov' class='external'>USGS</a> | <a href='https://www.usgs.gov/laws/policies_notices.html' class='external'>Policies</a>",
        params: {
          LAYERS: "0,1,2,3,4",
          TILED: true,
        },
...
app.measure = {
  ...
  formatLength: function (length) {
    const meters =
      length * app.map.getView().getProjection().getMetersPerUnit();
  ...
  addSegment: function () {
    ...
    app.measure.startMeasure();
    $$(".toast-text").html(
      "Total length: " + app.measure.formatLength(app.measure.measurement)
    );
  },
...
app.map = new ol.Map({
  ...
  layers: [app.layers.image, app.layers.measure, app.layers.position],
});
  ...
  iosChecks() {
    if (app.device.ios) {
      if (parseFloat(app.device.osVersion) < 11.3) {
        app.dialog.alert(
          "This app is not fully supported on devices running iOS < 11.3.",
          "Warning"
        );
      }
      if (!app.device.standalone) {
        if (!localStorage.getItem("dismissPrompt")) {
          app.toast
            .create({
              text:
                "Tap the <img src='assets/img/ios-share.png' height='18px'> button " +
                (app.device.ipad ? "at the top of the screen" : "below") +
    ...
    app.request({
      ...
      success: function (maps) {
        ...
        for (const map of maps) {
          ...
          $$("#map-list").append(li);
          if (
            app.utils.parseUrlQuery(document.URL).map &&
            map.name == app.utils.parseUrlQuery(document.URL).map
          ) {
        ...
        if (
          app.views.current.router.currentRoute.url == "/map/" &&
          !app.activeLayer
        ) {
          if (sessionStorage.getItem("settings")) {
            ...
            if (settings.basemap) {
              $$(
                "input[type=radio][name=basemap][value='" +
                  settings.basemap +
                  "']"
              )
                .prop("checked", true)
    ...
    for (const value of app.maps) {
      if (name == value.name) {
        ...
        app.layers.image.setSource(
          new ol.source.ImageStatic({
            ...
            attributions: value.attribution
              ? value.attribution.replace("<a", "<a class='external'")
              : null,
          })
        );
        ...
        app.map.getView().on("change:rotation", function (evt) {
          ...
          $$("#rotate-icon").css(
            "transform",
            "translate(-12px, -12px) rotate(" + degrees + "deg)"
          );
          if (radians == 0) {
            $$("#rotate-btn").css("display", "none");
          } else {
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({
            ...
            basemap:
              $$("input[name='basemap']:checked").val() != "none"
                ? $$("input[name='basemap']:checked").val()
                : null,
...
app.geolocation.on("change:position", function () {
  ...
  app.layers.position
    .getSource()
    .getFeatures()[0]
    .setGeometry(coordinates ? new ol.geom.Point(coordinates) : null);
});
